...
def elegir_limite():
    limite = input("¿A cuántos puntos juegan? (15 o 30): ")
    if limite not in ("15", "30"):
        print("Valor inválido, se jugará a 15.\n")
        return 15
    return int(limite)
...
def sumar_puntos(limite, equipo=None, pts=None):
    if equipo is None:
...
while True:
    ...
    # Ver si el usuario escribió un comando rápido tipo "n 2" o "e 3"
    if " " in op:
        partes = op.split()
        if len(partes) == 2:
            equipo_raw, pts_raw = partes
            equipo = normalizar_equipo(equipo_raw)
            try:
                pts = int(pts_raw)
                if equipo in puntos:
                    sumar_puntos(limite, equipo, pts)
                    continue
            except:
                pass
        print("Comando rápido inválido. Ej: 'n 2' o 'e 3'.")
        continue
